...
class StudentsModelViewSet(viewsets.ModelViewSet):
    ...
    Now in djangomodelpermission- admin and super user hae right to put,post,delete
    but user are not right to do, so, going to admin panel and give put,patch,post,delete permissions
    indivual(single action permission allow,)
    
    in DjangoModelPermissionsOrAnonReadOnly :- unauthunticated person can read our data
    but can't post,update or delet, for this user have to login
    '''